...
pub const APPLE_ENGAGEMENT_ID: &str = "agent-qa-eng-apple-itgc";
...
pub fn mock_transcription_result(input: &TranscribeAudioInput) -> TranscribeAudioResult {
    TranscribeAudioResult {
        transcript_text: "QA voice note: 30 minutes testing Apple ITGC controls.".to_string(),
...
pub fn mock_calendar_extract_result(
    ...
    let mut candidates = vec![
        CalendarExtractCandidate {
            ...
            extracted_text: "Control sync with Apple".to_string(),
            source_text: "Control sync with Apple, 11:30 AM - 12:00 PM".to_string(),
            confidence: 0.96,
            engagement_id: Some(APPLE_ENGAGEMENT_ID.to_string()),
            activity_id: Some(WALKTHROUGH_ACTIVITY_ID.to_string()),
            engagement_code: Some("A100".to_string()),
            engagement_name: Some("Apple ITGC".to_string()),
...
fn upsert_reference_codes(conn: &Connection) -> AppResult<()> {
    db::upsert_engagement(
        ...
        EngagementUpsertInput {
            id: Some(APPLE_ENGAGEMENT_ID.to_string()),
            code: Some("A100".to_string()),
            name: "Apple ITGC".to_string(),
            client: Some("Apple".to_string()),
            engagement_type: Some(EngagementType::External),
    ...
    db::upsert_activity(
        ...
        ActivityUpsertInput {
            ...
            engagement_id: APPLE_ENGAGEMENT_ID.to_string(),
...
mod tests {
    ...
    fn reset_and_seed_database_creates_reference_data_and_entries() {
        ...
        assert!(engagements
            .iter()
            .any(|engagement| engagement.id == APPLE_ENGAGEMENT_ID));
    ...
    fn mock_transcription_result_is_deterministic() {
        ...
        assert_eq!(
            ...
            "QA voice note: 30 minutes testing Apple ITGC controls."
        );
